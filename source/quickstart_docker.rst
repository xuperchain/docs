...
准备 docker 环境
^^^^^^^^
如果对 docker 比较熟悉的话，推荐使用 docker 搭建环境

安装好 docker 后，直接执行如下命令启动 xchain 容器

.. code-block:: bash
    :linenos:

    docker run -d --name xchain  xuper/xuperchain:latest


执行如下命令进入 xchain 容器，后续所有操作都可以在容器中执行 
...
也可以通过如下设置，直接在本地终端通过docker exec 执行容器中的命令

.. code-block:: bash
    :linenos:

        alias ./xchain-cli="docker exec -it xhcain"
