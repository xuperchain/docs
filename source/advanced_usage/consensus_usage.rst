...
PoW
^^^^^^
 类似于Bitcoin中的PoW机制。在集群中每个节点都以矿工身份进行挖矿，并且从其他节点同步区块。
...
变更矿工方法
-----------------
 XuperChain支持矿工的灵活变更，目前矿工的确定主要通过设置指定的候选人集合，矿工根据指定时间片在候选人集合中轮转产生。目前支持该变更方法的共识为TDPoS类共识(指TDPoS和XPoS)和PoA类共识(指PoA和XPoA)。
