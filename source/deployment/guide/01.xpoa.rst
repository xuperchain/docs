...
.. code-block:: shell
	...
	# 创建xuper链
	xchain-clii createChain
	# 启动服务节点
	nohup ./xchain &
	# check服务运行状况，修改-H后参数，可以查询每个节点状态
	for((i=1;i<=3;i++));do
	xchain-clii status -H 127.0.0.1:3710$i |grep -i height
	done
...
.. code-block:: shell
	...
	# 创建合约账号
	[work@]$ node1 -> xchain-clii account new --account 1111111111111111 --fee 1000 -H 127.0.0.1:37101
	# 执行结果
	# contract response:
	#         {
	#             "pm": {
	...
	# 给合约账号转账
	[work@]$ node1 -> xchain-clii transfer --to XC1111111111111111@xuper --amount 100000000 -H 127.0.0.1:37101
	# 执行结果
...
.. code-block::  shell
	...
	[work@]$ node1 -> xchain-clii wasm deploy --account XC1111111111111111@xuper --cname xpoa_validates --arg '{"addresss":"dpzuVdosQrF2kmzumhVeFQZa1aYcdgFpN;VSML7NenZnGZgCEwtbQDKDSrPHhT5wsu6","neturls":"127.0.0.1:47101;127.0.0.1:47102"}' ./xpoa_validates.wasm --fee 222065 -H 127.0.0.1:37101
	# 执行结果
...
.. code-block:: shell
	...
	[work@]$ node1 -> xchain-clii wasm invoke xpoa_validates --method add_validate --args '{"address":"bg3KLC3YCmvLWBCNAVHGHLfk3qeWEdoD3","neturl":"127.0.0.1:47103"}' --fee 300 -H 127.0.0.1:37101
	# 执行结果
...
.. code-block:: shell
	...
	[work@]$ node1 -> xchain-clii wasm invoke xpoa_validates --method get_validates -H 127.0.0.1:37101
	# 执行结果
...
.. code-block:: shell
	...
	[work@]$ node1 -> xchain-clii wasm invoke xpoa_validates --method update_validate -a '{"address":"bg3KLC3YCmvLWBCNAVHGHLfk3qeWEdoD3","neturl":"localhost:47103"}' --fee 300 -H 127.0.0.1:37101
	# 执行结果
...
.. code-block:: shell
	...
	[work@]$ node1 -> xchain-clii wasm invoke xpoa_validates --method del_validate -a '{"address":"bg3KLC3YCmvLWBCNAVHGHLfk3qeWEdoD3"}' --fee 300 -H 127.0.0.1:37101
	# 执行结果
