...
XuperChain 将虚拟机抽象为 InstaneCreator 接口约束
...
XuperBridge 负责管理合约上下文。每次合约运行都会有一个伴随合约执行的上下文(context)对象，context里面保存了合约的 kv cache对象，运行参数，输出结果，事件，日志等。context用于隔离多个合约的执行，也便于合约的并发执行。

合约执行的所有状态信息由 XuperBridge 的上下文管理器(ContextManager) 统一管理，虚拟机只需要关注无状态的合约执行。ContextManager 的主要功能，主要包括
...
context在合约虚拟机每次执行合约的时候创建。每个context都有一个context id，这个id由合约虚拟机维护，在xchain启动的时候置0，每次创建一个context对象加1，合约虚拟机保存了context id到context对象的映射。
...
4. 合约上下文信息的传递
 合约虚拟机和 XuperBridge 通过系统调用进行通信获取合约状态的通信与传递，具体包括
...
XuperBridge 通过合约执行沙盒(StateSandbox)技生成读写集，主要包括
...
3. 系统调用接口
    ...
    XuperChain 提供了通用的系统调用接口，按照系统调用的不同可以分为以下几类
...
原生合约支持
>>>>>>>>>>>>>>
XuperChain 也提供原生合约(也成native 合约)支持，当前支持通过 GO/JAVA 来进行智能合约开发，也可以方便地拓展到其他语言。

在原生合约中，每个合约是一个运行在本地的进程，合约和 xchain 之间通过 GRPC 协议进行通讯。需要注意的是，原生合约不支持 GAS 机制，安全性略弱。可以使用容器技术对原生合约可以访问的资源做基本的限制。
...
相比于 WASM 合约和 原生合约，以太坊合约在性能方面路弱，主要适用于已有业务迁移至xhcian 的场景。
 ...
 合约代码管理
 >>>>>>>>>>>>
 合约代码管理有 ContractCodeProvider 提供，主要接口约束为
...
合约部从请求中获取代码，合约调用时从账本获取代码
ContractCodeProvider 还维护了合约代码的缓存，当存在内存活着磁盘的缓存时，ContractCodeProvider 直接返回对应的代码缓存。
...
XVM 抽象 XVM 和 xchain 交互主要包括 code，context，call
    ...
    *  xvm_new_code/xvm_init_code/xvm_release_code

        xvm_new_code 和 xvm_init_code 实现了 code 管理。
        
        通常，一个 xvm_code_t 对应一个 WASM 模块，也对应一个动态链接库。

        xvm_new_code 主要完成动态链接库的打开，打开后从动态链接库中查找以下几个符号的地址

            * init_rt_ops: 用于初始化
            * init_import_funcs: 初始化外部函数
            * new_handle: 用于初始化一个 xvm_context_t 结构体

        xvm_init_code 则会调用 init_rt_ops 和 init_import_funcs 完成对应的初始化操作。 
        
        TODO
        将这两个过程分开的好处是 cgo 线程 
        如果使用了未初始化的 code 会怎么样


    * xvm_new_contex/xvm_init_context

        xvm_new_context 和 xvm_init_context 实现了 context 的初始化, new_handler 需要完成
        1.
        2.
        3.
        4.
